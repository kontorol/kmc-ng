...
import {DateUtil as Date} from './custom-moment';
...
export class Calendar implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    set yearRange(yearRange: string) {
        ...
        if (this.yearNavigator && yearRange) {
            ...
            this.populateYearOptions(yearStart - cal, yearEnd - cal);
        }
    }
    ...
    constructor(public el: ElementRef, public renderer: Renderer2, public browserService: BrowserService, public cd: ChangeDetectorRef, private zone: NgZone) {
    }

    ngOnInit() {
        const date = this.defaultDate || new Date();
        const locale = date.setJalali(this.browserService.getFromLocalStorage('kmc_lang') === 'fa');
        if (locale) {
            this._locale = locale;
        }
