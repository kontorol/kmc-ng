...
export class EntryComponent implements OnInit, OnDestroy {
    ...
    private _hideMenuItems(entry: KontorolMediaEntry,
        ...
        const isKontorolLiveStream = sourceType === KontorolSourceType.liveStream;
        ...
        return !(
            ...
            // hide live-dashboard menu item for entry that isn't kontorol live or has permission to access new analytics
            (isLiveDashboardCommand && (!isKontorolLiveStream || canAccessNewLiveAnalytics)) ||
						...
						switch (status.action) {
							...
							case ActionTypes.EntryLoaded:
                                ...
                                this._analyticsAllowed = this._analyticsNewMainViewService.isAvailable() // new analytics app is available
                                    && (
                                        this._sourceType !== KontorolSourceType.liveStream // and not a live stream
                                        || (this._sourceType === KontorolSourceType.liveStream // or it's a live stream and has permission
    ...
    public _openEntryAnalytics(): void {
        if (this._analyticsAllowed) {
            const route = this._sourceType === KontorolSourceType.liveStream ? 'analytics/entry-live' : 'analytics/entry';
